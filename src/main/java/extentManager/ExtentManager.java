...
public class ExtentManager {
	   ...
	   public static void createInstance() {
	      ...
	      // ExtentHtmlReporter htmlReporter = new ExtentHtmlReporter(REPORT_FILE_PATH+"Test-Automaton-Report"+formater.format(calendar.getTime())+".html");
	       ExtentSparkReporter htmlReporter = new ExtentSparkReporter(REPORT_FILE_PATH+"Test-Automaton-Report"+formater.format(calendar.getTime())+".html");
	       htmlReporter.config().setTheme(Theme.DARK);
	       htmlReporter.config().setEncoding("utf-8");
	       htmlReporter.config().setReportName("Automation Test Results");
	       htmlReporter.config().setEncoding("utf-8");
	       htmlReporter.config().setJS("$('.brand-logo').text('AutomationDemo');");
